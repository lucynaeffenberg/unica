<section class="py-50">
	<div class="container">
		<h2 class="text-center fs-30 fw-500 text-uppercase mb-0 mt-0">Our Services</h2>
		<p class="text-center">We provides the opportunity to prepare for life</p>
		<div class="flex-cols flex-cols-3 row-gap-30 mt-65">
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Art Studio</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Great Facility</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Activity Hub</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Fully Qualified</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Flexible Schedule</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
			<div>
				<div class="d-flex column-gap-30">
					<div>
						<img class="h-45" src="assets/img/icons/icon-walkman.svg" alt="">
					</div>
					<div>
						<h3 class="mt-0 fw-400 fs-24">Chemistry Lab</h3>
						<p>Lorem ipsum dolor sitdo amet, consectetur dont adipis elit. Vivamus interdum ultrices augue. Aenean dos cursus lania.</p>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>
